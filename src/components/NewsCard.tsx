...
export function NewsCard({ article, analysis, analysisMode, analysisLoading, onQuiz, onAnother, quizStarted, analysisLocked = false }: Props) {
  const canQuiz = !analysisLocked && !!analysis && !analysis.insufficient && !!analysis.quiz.question;
  return (
    <article className="news-card glass-panel card-arrive">
      ...
      {!analysisLocked && analysisMode === "unavailable" && <p className="inline-alert">원문 본문을 읽지 못했거나 AI 분석을 사용할 수 없어 퀴즈를 생성하지 못했습니다.</p>}
      {!analysisLocked && analysis?.insufficient && analysisMode !== "unavailable" && <p className="inline-alert">제공된 정보가 부족해 사실 확인 가능한 퀴즈를 만들 수 없습니다.</p>}
