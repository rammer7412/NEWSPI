...
export default function Home() {
  const { state, hydrated, secondsToNextTick, recordNewsView, registerAnalysis, cacheAnalysis, spendRouletteCoins, refundRouletteCoins, earnHappyCoin,
  ...
  function reflectMarketImpact(picked: NewsArticle, analyzed: AnalyzedNews) {
    ...
    const result = applyNewsImpact(picked.id, analyzed.marketImpact);
  ...
  function showAnalysis(picked: NewsArticle, analyzed: AnalyzedNews, mode: AnalysisMode) {
    setAnalysis(analyzed);
    setAnalysisMode(mode);
    const event = registerAnalysis(picked, analyzed, mode !== "unavailable");
    if (mode !== "unavailable" && (!event.active || event.resolved)) reflectMarketImpact(picked, analyzed);
  }
  ...
  async function spin() {
    ...
    try {
      ...
      if (cached) {
        showAnalysis(picked, cached, picked.isFallback ? "demo" : "ai");
        return;
      }
      ...
      try {
        ...
        showAnalysis(picked, analyzed.analysis, analyzed.mode);
        ...
        if (requestId !== spinRequest.current) return;
        showAnalysis(picked, unavailableFor(picked), "unavailable");
  ...
  return <div className="site-wrap">
    ...
    <main className="app-shell">
      ...
      {tab === "missions" && <DailyMissions state={state} onClaim={claimMission} onClaimAll={claimAllMissions} />}
      {tab === "happy" && <HappyDwi coins={state.coins} completedCount={state.happyTypingCount} dailyEarned={state.happyDailyEarned} spinCost={ROULETTE_COST} onEarn={earnHappyCoin} onGoHome={() => setTab("home")} />}
    </main>
