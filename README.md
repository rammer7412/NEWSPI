...
- 해킹 이벤트는 정상 분석이 가능한 기사에서만 결정됩니다. 기사별 판정, 베팅·정산, 가격 영향은 브라우저 저장 상태에서 한 번만 반영됩니다. 이슈와 방향을 모두 맞히면 베팅금의 2배를 반환하고, 방향만 맞히면 원금만 반환합니다. 방향이 다르면 베팅금을 잃습니다. 10 C 미만이면 베팅 없이 복구할 수 있으며 이 경우 미션은 진행되지 않습니다.
...
- 비정상적인 `localStorage` 값은 안전한 초기 상태로 정규화합니다. 기존 저장 데이터는 버전 2 형식으로 이전하며, 코인·보유 자산·퀴즈 보상 기록을 유지합니다. 새 기록에는 기사 전문을 저장하지 않습니다.
...
| `src/hooks/useUserState.ts`, `src/lib/storage.ts` | 보상·거래·기록·localStorage 상태 및 이전 |
...
- 거래 대상은 실제 기업 주식이 아닌 **7개 가상 이슈 지수**입니다. 1분 변동은 브라우저에서 이슈별 -0.5~+0.5%로 계산하고, 기사 반영 폭은 AI 분석의 중요도 값에 따라 0.5~5%로 제한합니다. 최소 가격은 10 C입니다. 실제 시장 시세가 아니며 브라우저 간 가격은 동기화되지 않습니다.
